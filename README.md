...
- **主な機能**:
  ...
  - **店舗別ルール適用とAI推測の視覚的区別（マーク表示）** ← NEW!
  - 仕訳データの手動編集
  - 店舗別ルール管理（部分一致検索）
  - **店舗別ルールのエクスポート・インポート（JSON形式）**
  - 勘定科目マスタ管理（追加・編集・削除）
  - 最新ルールで更新（バッチ処理による高速処理）
  - Excelファイル出力（.xlsx形式）
...
#### ルールのエクスポート・インポート
- **エクスポート**: 「エクスポート」ボタンをクリック → JSON形式でダウンロード
  - ファイル名: `store_rules_YYYY-MM-DD.json`
  - 他の環境やバックアップとして保存可能
  
- **インポート**: 「インポート」ボタンをクリック → JSONファイルを選択
...
## ✅ 実装済み機能
1. ✅ CSVファイルアップロード・解析（楽天カードCSV、Shift_JIS対応）
2. ✅ AI推測エンジン（店名から勘定科目・摘要を推測）
3. ✅ 学習ルール（部分一致検索）
4. ✅ 仕訳データ編集UI（表形式、手動修正可能）
5. ✅ **店舗別ルール適用とAI推測の視覚的区別（✅ルール適用、🤖AI推測）** ← NEW!
6. ✅ 店舗別ルール管理画面（追加・編集・削除）
7. ✅ 店舗別ルールのエクスポート・インポート（JSON形式）
8. ✅ 勘定科目マスタ管理（追加・編集・削除、システム科目も対応）
9. ✅ 最新ルールで一括更新機能（バッチ処理による高速化）
10. ✅ Excelファイル出力（.xlsx形式、支払月対応）
11. ✅ プログレスバー表示（処理進捗の可視化）
...
3. 仕訳データの履歴管理
4. 月次・年次レポート機能
5. AI推測精度の向上（より多くのパターン対応）
6. 勘定科目マスタのエクスポート・インポート
