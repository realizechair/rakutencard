...
app.post('/api/parse-csv', async (c) => {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const rule = await env.DB.prepare(
      'SELECT * FROM learning_rules WHERE store_name = ?'
    ).bind(storeName).first<LearningRule>()
...
<body class="bg-gray-50">
    <div class="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <button onclick="showAccountModal()" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
                <i class="fas fa-plus mr-2"></i>勘定科目を追加
            </button>
            <div id="account-list" class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
    ...
    <div id="account-modal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <h3 class="text-xl font-bold mb-4">勘定科目を追加</h3>
            <div class="space-y-4">
                <div>
            ...
            <div class="flex justify-end space-x-3 mt-6">
                ...
                <button onclick="addAccountSubject()" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">追加</button>
            </div>
        </div>
    </div>
